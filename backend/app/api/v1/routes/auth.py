...
from app.core.deps import CurrentUserDep, SessionDataDep, get_client_ip, get_db, get_redis_client
...
async def register(
    ...
    data = await service.register(payload, client_ip=get_client_ip(request))
    return ApiResponse(
        success=True,
        data=data,
...
async def login(
    ...
    data, session_id = await service.login(
        email=payload.email,
...
@router.patch("/me", response_model=ApiResponse[MeResponseData])
async def update_me(
...
async def resend_verification(
    ...
    return ApiResponse(
        success=True,
        data=None,
        message="E-mel pengesahan telah dihantar semula",
    )
